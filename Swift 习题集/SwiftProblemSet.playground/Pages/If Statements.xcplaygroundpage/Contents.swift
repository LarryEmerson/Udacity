...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    var okToRelease:Bool = true
    if(bugs>=10){
        okToRelease=false
        print("10 or more bugs were found, plz fix them before next review!")
    }
    if(!hasMusic){
        okToRelease=false;
        print("No music were found, plz add music before next review!")
    }
    if(numberOfLevels<=5){
        okToRelease=false;
        print("Less than 5 levels were found, plz add more before next review!")
    }
    if(okToRelease){
        release()
    }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) { 
    if(!bike){
        print(name + " can not finish bike program")
    }else if(!run){
        print(name + " can not finish run program")
    }else{
        print(name + " is ready")
    }
}
